...
class Eaf:
    ...
    def add_ref_annotation(self, id_tier, tier2, time=None, value='',
                           prev=None, svg=None):
        ...
        if self.tiers[tier2][0]:
            for aid, (begin, end, _, _) in self.tiers[tier2][0].items():
                ...
                if begin <= time and end >= time:
                    ann = aid
                    break
        # tier2 is a reference tier
        if self.tiers[tier2][1] and time is None:
            n = len(self.tiers[id_tier][1])
            if n == 0:
                ann = next(iter(self.tiers[tier2][1]))
            else:
                ann = next(islice(self.tiers[tier2][1], n, n + 1))
        # tier2 is a reference tier and time is specified,
        # so we grab the top tier in the hierarchy
        else:
